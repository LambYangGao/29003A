...
struct serial_handle {
    int fd;                   // 文件描述符 
    bool use_termios_timeout; // 是否使用终端IO超时

    struct {
        int c_errno;          // 错误码
        char errmsg[96];      // 错误消息缓冲区
    } error;
};
...
int serial_open_WF3588(serial_t* serial, const char* path, uint32_t baudrate) {
    return serial_open_advanced_WF3588(serial, path, baudrate, 8, PARITY_NONE, 1, false, false);
}
...
int serial_open_advanced_WF3588(serial_t* serial, const char* path, uint32_t baudrate, unsigned int databits, serial_parity_t parity, unsigned int stopbits, bool xonxoff, bool rtscts) {
    /* Validate args */
    if (databits != 5 && databits != 6 && databits != 7 && databits != 8)
        return _serial_error(serial, SERIAL_ERROR_ARG, 0, "Invalid data bits (can be 5,6,7,8)");
    if (parity != PARITY_NONE && parity != PARITY_ODD && parity != PARITY_EVEN)
    ...
    /* 配置波特率和校验位 */
    struct user_config config_ = { 0 };
    config_.baud = baudrate;
    config_.verify = 0;
    /* Set termios attributes */
	if (ioctl(serial->fd, IOCWREG, &config_) < 0) {
		int errsv = errno;
		close(serial->fd);
		serial->fd = -1;
		return _serial_error(serial, SERIAL_ERROR_CONFIGURE, errsv, "Setting serial port attributes");
...
int serial_read_WF3588(serial_t* serial, uint8_t* buf, size_t len, int timeout_ms) {
    printf("serial_read_WF3588!\r\n");
    ssize_t ret;
...
#define MAX_DATA_LENGTH 256
    ...
    FD_ZERO(&rd);
    FD_SET(serial->fd, &rd);
    fd_sel = select(serial->fd + 1, &rd, NULL, NULL, &tv_timeout);
    ...
    else
    {
        ...
        ret = FD_ISSET(serial->fd, &rd);
        //printf("FD_ISSET ret is %d\r\n", ret);
        if (ret)
        {
            memset(buffer, 0, sizeof(buffer));
            bytes_read = read(serial->fd, buf, 14);
            printf("%d\r\n", bytes_read);
            //uart0_handle(buf, &uart0_data);
            int r_num = 14;
            //if(0)
