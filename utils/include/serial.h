...
int serial_open(serial_t* serial, const char* path, uint32_t baudrate);             // 默认设置打开串口
int serial_open_WF3588(serial_t* serial, const char* path, uint32_t baudrate);

int serial_open_advanced(serial_t* serial, const char* path,                        // 自定义设置打开串口
                         uint32_t baudrate, unsigned int databits,
                         serial_parity_t parity, unsigned int stopbits,
                         bool xonxoff, bool rtscts);                             
int serial_open_advanced_WF3588(serial_t* serial, const char* path,
    uint32_t baudrate, unsigned int databits,
    serial_parity_t parity, unsigned int stopbits,
    bool xonxoff, bool rtscts);

int serial_read(serial_t *serial, uint8_t *buf, size_t len, int timeout_ms);        // 带超时读取数据
