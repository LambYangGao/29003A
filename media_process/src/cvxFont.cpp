...
void cvx::CvxFont::putTextStr(cv::Mat &img, const cv::String &text, cv::Point pos, const cv::Scalar &color)
{
    ...
    int ret;
    wchar_t wc;
    for (wchar_t wc; (ret = mb2wc(&wc, ptr, end - ptr)) > 0; ptr += ret)
    {
        ...
        for (auto j = 0; j < cols; ++j)
        {
            ...
            if (slot->bitmap.buffer[off] & (0x80 >> (j % 8)))
            {
                ...
                if (r >= 0 && r < img.rows && c >= 0 && c < img.cols)
                {
                    ...
                    // merge set color with origin colors
                    for (int k = 0; k < 3; ++k)
                    {
                        scalar.val[k] = static_cast<uchar>(color.val[k]);
                    }
...
int cvx::mb2wc(wchar_t *dest, const char *src, size_t n)
{
    if (n == 0 || src == nullptr)
    {
        return 0; // 输入为空，不进行转换
    ...
    uint32_t firstByte = static_cast<uint8_t>(*src);
    int numBytes = 0;
