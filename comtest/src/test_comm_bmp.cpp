...
inline RK_U32 OSD_MAKECOLOR_ARGB(RK_U8 r, RK_U8 g, RK_U8 b, OSD_COMP_INFO compinfo)
{
    ...
    pixel |= (r1 << compinfo.alen | (g1 << (compinfo.rlen + compinfo.alen)) | (b1 << (compinfo.rlen + compinfo.glen + compinfo.alen)));
    return pixel;
}
...
RK_S32 load_bmp_ex(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    ...
    pVideoLogo->height = (RK_U16)((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight : (-bmpInfo.bmiHeader.biHeight));
    w = pVideoLogo->width;
    h = pVideoLogo->height;
